...
CHECKSUM_512_FILE="${ARCHIVE_DIR}/checksums-512.txt"
...
for binary_path in "$COMPILED_DIR"/*; do
    ...
    final_binary_name="hello"
    if [[ "$filename" == *".exe" ]]; then
        final_binary_name="hello.exe"
    fi
...
echo -e "${CYAN}🔐 Generating checksums 512...${NC}"
(
  cd "$ARCHIVE_DIR" || exit 1
  for file in *; do
    if [ "$file" != "checksums-512.txt" ]; then
      shasum -a 512 "$file"
    fi
  done
) > "$CHECKSUM_512_FILE"
